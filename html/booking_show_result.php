...
<div id="search_box">
  ...
  if($catagory=='booking_name'){
        $catname = '예약 환자명';
    } else if($catagory=='booking_num'){
        $catname = '예약 번호';
    } else if($catagory=='doctor_id'){
        $catname = '담당의사';
    }else if($catagory=='booking_date'){
        $catname = '예약일';
    }
?>
...
<table>
   <tr>
        ...
        <th class="booking_name"     >환자 이름    </th>
...
<?php
	...
	$query = $db->query("select * from booking$bid where $catagory like '$search_con' order by booking_array desc limit $start, $numLines");
...
<tr>	
	...
	<br>
		...
		$numRecords = $db->query("select count(*) from booking$bid where $catagory like '$search_con'")->fetchColumn();
